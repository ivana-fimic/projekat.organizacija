...
public class SOZapamtiClanaTest {
    ...
    public SOZapamtiClanaTest() {
    }
    @Mock
    private DBBroker dbBroker;
    private SOZapamtiClana so;
    private Clan clan;

    @BeforeEach
    public void setUp() throws NoSuchFieldException, IllegalArgumentException, IllegalAccessException {
        so = new SOZapamtiClana();
    ...
    public void testExecuteUspesno() throws Exception {
        ...
        doNothing().when(dbBroker).update(clan);

        assertDoesNotThrow(() -> so.execute(clan));

        verify(dbBroker, times(1)).update(clan);
    }
